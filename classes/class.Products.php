        ...
        if( $result = $this->mysqli->query($q) ) :
            while( $row = $result->fetch_assoc() ):
                ...
                $resultArray = array(   'id'                    => $row['virtuemart_product_id'],
                                        ...
                                        'paragraph'             => $row['product_paragraph'],
...
*/
    public function createItem( $array, $design ) {
        ...
        $paragraph_count        = strlen($array['paragraph']);
        ...
        $last_save              = NULL;
        if( !empty($array['last_save']) ):
                $formatted_date         = date( 'd-m-Y' , strtotime($array['last_save']));
                $last_save              = 'Modified: <span style="font-weight: 600">'. $formatted_date .'</span>';
        endif;
                ...
                {$itemstyle}
                    <div class="item--data">
                        <div class="item--text" id="item-{$array['id']}" data-realid="{$array['real_id']}">
                            ...
                            <div>Paragraph character count: {$paragraph_count}</div>
            ...
            $output = <<<HTML
                <div class="item" style="height: auto !important;">
                    ...
                    <div class="item--pdfs">
                        ...
                        <form enctype="multipart/form-data" id="formupload{$array['id']}" class="formupload formupload{$array['id']}" data-id="{$array['id']}" method="post" style="margin-top: 10px">
                            <input type="hidden" value="{$array['id']}" name="id">
                            <input type="hidden" value="{$array['id']}" name="test">
                ...
                if( $part == 'details' ):
                    $output .= <<<HTML
                        ...
                        <div class="tiny-paragraph">
                            <textarea id="tinymce-paragraph">{$product_paragraph}</textarea>
                        </div>
        ...
        if( $result = $this->mysqli -> query( $q ) ) :
            while( $row = $result->fetch_assoc() ) :
                ...
                $output                .= <<<HTML
                    ...
                    <div class="tiny-paragraph">
                        <textarea id="tinymce-paragraph_nl">{$product_paragraph_nl}</textarea>
                    </div>
                    <!--
